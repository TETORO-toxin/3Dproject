    ...
    if (equippedWeapon_ != Game::WeaponType::None) {
        ...
        if (wh != -1) {
            ...
            if (fh != -1 && baseModelHandle_ != -1) {
                ...
                // Use only the frame world position as the base position for the weapon
                // and apply the weapon's equipOffset rotated by the player's yaw. Do not
                // use the frame axes for weapon orientation to avoid flips caused by
                // wrist/forearm twisting.
                VECTOR framePosOnly = VGet(fm.m[3][0], fm.m[3][1], fm.m[3][2]);
                pickedFramePos = framePosOnly;
                havePickedFramePos = true;

                // Rotate equipOffset by player's yaw around Y so offset follows player facing
                VECTOR off = spec.equipOffset;
                float cosy = cosf(currentYaw_);
                float siny = sinf(currentYaw_);
                VECTOR rotatedOff = VGet(off.x * cosy - off.z * siny,
                                       off.y,
                                       off.x * siny + off.z * cosy);

                VECTOR wpos = VAdd(framePosOnly, rotatedOff);

                ::MV1SetPosition(wh, wpos);
                ::MV1SetScale(wh, VGet(spec.equipScale, spec.equipScale, spec.equipScale));
                // Build rotation as: equipRotation.x, currentYaw_ + equipRotation.y, equipRotation.z
                float d2r = DX_PI_F / 180.0f;
                float rx = spec.equipRotation.x * d2r;
                float ry = currentYaw_ + spec.equipRotation.y * d2r;
                float rz = spec.equipRotation.z * d2r;
                ::MV1SetRotationXYZ(wh, VGet(rx, ry, rz));
                ::MV1DrawModel(wh);
            } else {
